...
import { useApproveLeave, useRejectLeave } from "./useLeaves";
...
export function LeaveApprovalActions({ leaveId, version }: LeaveApprovalActionsProps) {
  ...
  function handleApprove() {
    approveMutation.mutate({ id: leaveId, version });
  }
  ...
  function handleReject(e: FormEvent) {
    ...
    rejectMutation.mutate(
      { id: leaveId, reason: rejectReason.trim(), version },
      {
        onSuccess: () => {
          setOpen(false);
          setRejectReason("");
  ...
  return (
    <div className="flex gap-2">
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle>休暇申請を却下</DialogTitle>
            <DialogDescription>却下理由を入力してください。</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleReject}>
            <div className="space-y-2 py-2">
              <Label htmlFor="leaveRejectReason">却下理由</Label>
              <textarea
